...
#if defined(NDEBUG)
#error "Shit cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ushit) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zSHIT is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSHIT spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSHIT without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSHIT spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zSHIT spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state, bool fWitnessEnabled)
{
    ...
    for (const CTxIn& txin : tx.vin) {
...

            {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                        return state.Invalid(error("AcceptToMemoryPool : zSHIT spend tx %s already in block %d",
                    ...
                    for (const CTxIn& txIn : tx.vin) {
                        ...
                        if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                            return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                     tx.GetHash().GetHex()),
                                REJECT_INVALID, "bad-txns-invalid-zshit");
                    }
                } else {
                    LOCK(pool.cs);
                    ...
                    // Check that zSHIT mints are not already known
                    if (tx.IsZerocoinMint()) {
                        for (auto& out : tx.vout) {
                            if (!out.IsZerocoinMint())
                                continue;
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Shit
        ...
        {
            RenameThread("shit-scriptch");
            scriptcheckqueue.Thread();
        }

        void RecalculateZSHITMinted()
        {
        ...
        void RecalculateZSHITSpent()
        {
            ...
            while (true) {
                ...
                //Remove spends from zSHIT supply
        ...
        bool RecalculateSHITSupply(int nHeightStart)
        {
        ...
        bool UpdateZSHITSupply(const CBlock& block, CBlockIndex* pindex)
        {
            std::list<CZerocoinMint> listMints;
            BlockToZerocoinMintList(block, listMints);
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            UpdateZSHITSupply(block, pindex);
            ...
            //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSHITSpent: %s\n",
 ...
 */
        ...
        void static UpdateTip(CBlockIndex * pindexNew)
        {
            ...
            // If turned on AutoZeromint will automatically convert SHIT to zSHIT
 ...
 */
        ...
        bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
        {
            ...
            if (nHeight > Params().Zerocoin_StartHeight()) {
                ...
                for (const CTransaction& tx : block.vtx) {
                    ...
                    // double check that there are no double spent zSHIT spends in this block
                    if (tx.IsZerocoinSpend()) {
                        for (const CTxIn txIn : tx.vin) {
                            if (txIn.scriptSig.IsZerocoinSpend()) {
                                libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                                if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                                    return state.DoS(100, error("%s : Double spending of zSHIT serial %s in block\n Block: %s",
        ...
        bool ProcessNewBlock(CValidationState & state, CNode * pfrom, CBlock * pblock, CDiskBlockPos * dbp)
        {
            ...
            if (nMints || nSpends)
                LogPrintf("%s : block contains %d zSHIT mints and %d zSHIT spends\n", __func__, nMints, nSpends);
        ...
        bool static ProcessMessage(CNode * pfrom, string strCommand, CDataStream & vRecv, int64_t nTimeReceived)
        {
            ...
            if (strCommand == NetMsgType::VERSION) {
                ...
                if (pfrom->cleanSubVer == "/Shit Core:1.1.0.0/" ||
					pfrom->cleanSubVer == "/Shit Core:1.2.0.0/" ||
					pfrom->cleanSubVer == "/Shit Core:1.2.0.1/" ||
					pfrom->cleanSubVer == "/Shit Core:1.3.0.0/" ||
					pfrom->cleanSubVer == "/Shit Core:1.3.0.1/" ||
                    pfrom->cleanSubVer == "/Shit Core:1.3.1.0/") { // Archived pivx 2.3 source
                    LOCK(cs_main);
                ...
                // Shit: We use certain sporks during IBD, so check to see if they are
