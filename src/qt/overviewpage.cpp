...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SHIT)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount shitAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance + watchImmatureBalance;    
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // Combined balances
	CAmount availableTotalBalance = shitAvailableBalance;
	CAmount sumTotalBalance = nTotalBalance;

    // SHIT labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnitComma(nDisplayUnit, shitAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    QString automintHelp = tr("Current percentage of zSHIT.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", false);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in shit.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in shit.conf");
    }
    ...
    bool showWatchOnlySHITPending = watchUnconfBalance != 0;
    bool showSHITLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySHITLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;

    ui->labelBalance->setVisible(showSHITAvailable || showWatchOnlySHITAvailable);
    ui->labelBalanceText->setVisible(showSHITAvailable || showWatchOnlySHITAvailable);
    ui->labelWatchAvailable->setVisible(showSHITAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showSHITPending || showWatchOnlySHITPending);
    ui->labelPendingText->setVisible(showSHITPending || showWatchOnlySHITPending);
    ui->labelWatchPending->setVisible(showSHITPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showSHITLocked || showWatchOnlySHITLocked);
    ui->labelLockedBalanceText->setVisible(showSHITLocked || showWatchOnlySHITLocked);
    ui->labelWatchLocked->setVisible(showSHITLocked && showWatchOnly);
    ui->labelImmature->setVisible(showImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelImmatureText->setVisible(showImmature || showWatchOnlyImmature);
    ui->labelWatchImmature->setVisible(showImmature && showWatchOnly);
	bool showzSHITAvailable = settingShowAllBalances; 
	bool showzSHITUnconfirmed = settingShowAllBalances;
	bool showzSHITImmature = settingShowAllBalances;
...
void OverviewPage::timerTickSlot()
{
    ...
    QNetworkReply* replyEx = managerEx.get(QNetworkRequest(QUrl(QString("https://api.shitcoind.com/exchange2/%1.png").arg(unixtime))));
...
// TFinch has edited it to work with Shit
...
void OverviewPage::updatBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced()) {
        ...
        double BlockReward = GetBlockValue(chainActive.Height());
        double BlockRewardshitcoin = static_cast<double>(BlockReward / COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardshitcoin, 'f', 1));
        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" SHIT"));
